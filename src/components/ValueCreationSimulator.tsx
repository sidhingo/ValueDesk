...
const ValueCreationSimulator = () => {
  ...
  if (display) {
    ...
    // Performance Upside: (Total Value Created / Entry EV)
    performanceUpside = entryEV !== 0 ? (totalValueCreated / entryEV) * 100 : 0;
  }

  const formatCurrency = (val) => Math.round(val).toLocaleString();

  const labelStyle = "block text-[10px] font-bold text-[#64748B] uppercase tracking-[0.1em] mb-2 h-[42px] flex items-end overflow-hidden";
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-10 pb-10 font-sans bg-white text-left">
          ...
          {!display ? (
            ...
            <div>
              <div className="text-center space-y-4">
                ...
                <div className="text-6xl font-light tracking-tighter text-[#1E293B]">
                  ${formatCurrency(totalValueCreated)}M
                </div>
                <div className={`text-[11px] font-black uppercase tracking-[0.2em] px-4 py-2 inline-block border-2 ${performanceUpside > 0 ? 'border-emerald-500/20 text-emerald-700 bg-white' : 'border-red-500/20 text-red-700 bg-white'}`}>
                  {performanceUpside > 0 ? '▲' : '▼'} {performanceUpside.toFixed(1)}% Performance Upside
                </div>
              </div>
              
              <div className="mt-10 pt-8 border-t border-[#E2E8F0] space-y-4">
                 ...
                 <div className="flex justify-between text-[10px] uppercase tracking-widest text-[#64748B]">
                   <span>Implied Exit Enterprise Value:</span>
      ...
      {/* UPDATED CALCULATION LOGIC & STRATEGY */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 border-t border-[#E2E8F0] pt-12">
        <div className="space-y-8">
          ...
          <div className="space-y-6 text-left">
            <div className="border-b border-[#F1F5F9] pb-4">
              ...
              <p className="text-[12px] font-mono text-[#1E293B] bg-[#F8FAFC] p-2">(Exit EBITDA - Entry EBITDA) × Entry Multiple</p>
            </div>
            <div className="border-b border-[#F1F5F9] pb-4">
              ...
              <p className="text-[12px] font-mono text-[#1E293B] bg-[#F8FAFC] p-2">Exit EBITDA × (Exit Multiple - Entry Multiple)</p>
            </div>
            <div>
              <span className="text-[11px] font-bold text-[#475569] uppercase block mb-1">Performance Upside</span>
              <p className="text-[12px] text-[#64748B] mb-2 leading-relaxed">Represents the total value created as a percentage increase over the initial entry enterprise value.</p>
              <p className="text-[12px] font-mono text-[#1E293B] bg-[#F8FAFC] p-2">(Total Value Created / Entry Enterprise Value) × 100</p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-8 text-left">
          ...
          <div className="space-y-6 text-left">
            <div className="border-l-4 border-[#1E293B] pl-4">
              ...
              <p className="text-[13px] text-[#64748B] leading-relaxed">Increasing profits through structural efficiency, offshoring, and streamlined management.</p>
            </div>
            <div className="border-l-4 border-[#1E293B] pl-4">
            ...
            <div className="border-l-4 border-[#1E293B] pl-4">
              ...
              <p className="text-[13px] text-[#64748B] leading-relaxed">Optimizing product mix and pricing power to drive profitability faster than revenue growth.</p>
            </div>
          </div>
        </div>
      </div>
